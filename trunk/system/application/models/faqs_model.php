...
class Faqs_Model extends Model {
    ...
    function update($id, $data){
        $this->db->update('faqs', $data, array('id' => $id));
    }
    
    function get_by_id($id){
        $row = $this->db->getwhere('faqs', array('id' => $id));      
    ...
    function search($params, $extra_where = '') {
		...
		if(isset($params['question']) && $params['question'] != '') {
			$where .= ' AND question LIKE "%' . $params['question'] . '%"';
		}
		if(isset($params['answer']) && $params['answer'] != '') {
			$where .= ' AND answer LIKE "%' . $params['answer'] . '%"';
		}
		...
		if(isset($params['reply']) && $params['reply'] != '') {
			if($params['reply'] == 0) {
				$where .= " AND (TRIM(answer) = '' OR answer IS NULL)";
			}
			else if($params['reply'] == 1) {
				$where .= " AND TRIM(answer) <> ''";
			}
		}
        if($extra_where != '') {
			$where .= " $extra_where";
    ...
    function delete_by_id($id){
		$row = $this->db->delete('faqs', array('id' => $id));		
	}
}
?>